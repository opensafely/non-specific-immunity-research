...
ds *lrti*, has(type float)				// Weekly LRTI flags

egen n_lrti = rownonmiss(`r(varlist)')
summ n_lrti, d
summ n_lrti if n_lrti > 0, d
drop n_lrti

ds *lrti*, has(type float)

egen min_lrti = rowmin(`r(varlist)')	// Use first LRTI for now
format %td min_lrti
...
bysort msoa: gen count = _N
summ count, d

egen n_utla = tag(utla)
count if n_utla

bysort utla: gen count1 = _N
summ count1, d
